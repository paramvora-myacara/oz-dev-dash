// src/components/AerialImagesSection.tsx
'use client';

import { useState, useEffect } from 'react';
import Image from 'next/image';
import { Plane, ChevronLeft, ChevronRight, ZoomIn, Download, Plus } from 'lucide-react';
...
interface AerialImagesSectionProps {
  isEditMode?: boolean;
  listingSlug?: string;
}

export default function AerialImagesSection({ 
  isEditMode = false,
  listingSlug = ''
}: AerialImagesSectionProps) {
  ...
  useEffect(() => {
    async function loadImages() {
      try {
        const aerials = await getAvailableImages(projectId, 'details/property-overview/aerial-images');
        setImages(aerials);
      } catch (error) {
  ...
  const handleImagesChange = async () => {
    try {
      ...
      console.error('Error reloading aerial images:', error);
    }
  };
    ...
    <section className="py-16 px-4 md:px-8 bg-gray-50 dark:bg-gray-900/50">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
            High-altitude views showcasing the property's scale, connectivity, and surrounding environment.
          </p>
        </div>

        <div className={`relative group bg-white dark:bg-gray-800 rounded-3xl p-4 shadow-xl border border-gray-100 dark:border-gray-700 transition-all duration-300 ${
        ...
        onClick={images.length === 0 && isEditMode ? () => setIsImageManagerOpen(true) : undefined}>
          ...
          {images.length > 0 ? (
            ...
            <div className="aspect-[21/9] bg-gray-50 dark:bg-gray-900/50 rounded-2xl flex flex-col items-center justify-center text-gray-400">
              <Plus size={48} className="mb-4" />
              <p className="text-lg font-medium">Add Aerial Perspective Images</p>
            </div>
          )}
      ...
      {isEditMode && listingSlug && (
        <ImageManager
          ...
          defaultCategory="details/property-overview/aerial-images"
        />
      )}
    </section>
  );
