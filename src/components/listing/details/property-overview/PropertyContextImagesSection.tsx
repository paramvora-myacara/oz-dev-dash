// src/components/listing/details/property-overview/PropertyContextImagesSection.tsx
'use client';
...
interface PropertyContextImagesSectionProps {
  isEditMode?: boolean;
  listingSlug?: string;
}

export default function PropertyContextImagesSection({ 
  isEditMode = false,
  listingSlug = ''
}: PropertyContextImagesSectionProps) {
  ...
  const loadImages = useCallback(async () => {
    if (!projectId) return;
    try {
      const propertyImages = await getAvailableImages(projectId, 'details/property-overview/property-context-images');
      setImages(propertyImages);
    } catch (error) {
      console.error('Error loading property context images:', error);
    } finally {
      setLoading(false);
    }
  }, [projectId]);
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 shadow-sm border border-gray-100 dark:border-gray-800 mb-8">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
          Detailed visual overview of the property's design, architecture, and features.
        </p>
      </div>
        ...
        {images.length > 0 ? (
          ...
          >
            ...
            <Image
              ...
              alt={`Property context view ${currentIndex + 1}`}
              fill
          ...
          <div className="aspect-[21/9] bg-gray-50 dark:bg-gray-900/50 rounded-2xl flex flex-col items-center justify-center text-gray-400 border-2 border-dashed border-gray-200 dark:border-gray-700">
            ...
            <p className="text-lg font-medium">Add Property Context Images</p>
          </div>
        )}
      ...
      {isEditMode && listingSlug && (
        <ImageManager
          ...
          defaultCategory="details/property-overview/property-context-images"
        />
      )}
    </div>
  );
