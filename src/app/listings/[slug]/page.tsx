...
// A mapping from icon names to Lucide components
...
function ListingPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    if (listingData) {
      ...
      document.title = listingData.listingName;
    }
  }, [params.slug]);
  ...
  useEffect(() => {
    async function loadHeroImages() {
      if (!listing) return;
      try {
        const images = await getRandomImages(listing.projectId, 'general', 5);
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-white border border-gray-200 dark:border-white/20 backdrop-blur-xl shadow-sm hover:shadow-md transition-shadow duration-200">
                ...
                ${minInvestment / 1000}K Minimum Investment
              </span>
              <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-white border border-gray-200 dark:border-white/20 backdrop-blur-xl shadow-sm hover:shadow-md transition-shadow duration-200">
        ...
        <section className="h-[30vh] sm:h-[40vh] md:h-[50vh] relative overflow-hidden px-4 md:px-8">
          <div className="absolute inset-0">
            {heroImages.length > 0 ? (
              ...
              <Image
                src="/property-hero.jpg" // Fallback image
                alt={`${listingName} property image`}
                fill
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              {investmentCards.map((card, idx) => {
                ...
                return (
                    <Link
                    key={idx}
                    href={`/listings/${listing.listingSlug}/details/${card.id}`}
                    ...
                    <div className="relative flex items-center justify-between mb-6">
                        ...
                        <h3 className={`text-2xl font-semibold ${style.textColor}`}>
                            {card.title}
                        </h3>
                        </div>
                    ...
                    <div className="space-y-8 mb-6 flex-1">
                        ...
                        <div key={metricIdx} className="flex items-center justify-between">
                            <span className={`text-lg font-medium ${style.accentColor}`}>
                            {metric.label}
                            </span>
                    ...
                    <p className={`text-base leading-relaxed font-light ${style.accentColor}`}>
                        {card.summary}
                    </p>
                    </Link>
                )
...
export default function ListingPageWrapper({ params }: { params: { slug: string } }) {
    return (
