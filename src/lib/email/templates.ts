...
export function getTemplate(outcome: string, data: TemplateData): { subject: string; html: string } {
  ...
  switch (outcome) {
    case 'pending_signup': {
      subject = `Following Up on Our Conversation - OZListings`;

      const promoCode = `${data.callerName.toUpperCase()}-OZL-2026`;
      ...
      content = `
                    ...
                    <p style="font-size:15px;line-height:24px;margin:0 0 16px 0;color:#4b5563;">As a special thank you for our chat, please use the promo code <strong style="color:#1e88e5;">${promoCode}</strong> during sign-up. This will make your subscription <strong>completely free until June 1st</strong>.</p>
    ...
    case 'no_answer': {
      subject = 'Tried Reaching You - OZListings';
      const promoCode = `${data.callerName.toUpperCase()}-OZL-2026`;
      const signupUrl = `https://ozlistings.com/developers?utm_promo=${promoCode}`;
      content = `
                ${greeting}
    ...
    case 'invalid_number': {
      subject = 'Trying to Reach You - OZListings';
      const promoCode = `${data.callerName.toUpperCase()}-OZL-2026`;
      const signupUrl = `https://ozlistings.com/developers?utm_promo=${promoCode}`;
      content = `
                ${greeting}
                ...
                <p style="font-size:15px;line-height:24px;margin:0 0 16px 0;color:#4b5563;">I wanted to let you know that OZListings is running a special promotion that allows you to list your project on our marketplace <strong>fully free through June 1st, with no fees (onboarding or otherwise).</strong></p>
