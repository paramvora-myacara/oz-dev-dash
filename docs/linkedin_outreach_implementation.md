...
### 2.2. Company Rotation (Spam Mitigation)
- **Daily Limit**: 20 connection requests per account per day.
- **Firm Rotation**: Limit outreach to **one person per firm per day** globally across all Oz Listing accounts.
- **Selection Rule**: The system skips all contacts from a firm if *any* Oz Listing account has already successfully sent a connection to *any* contact at that firm today.

### 2.3. Hybrid Selection (Tonight's Batch)
- **6:00 PM (Selection Phase)**: A background script runs to identify the "Best 40" contacts (20 for Jeff, 20 for Todd). These are moved from `pending` to `queued`.
- **6:00 PM - 6:30 PM (Review Phase)**: The `oz-dev-dash` dashboard displays "Tonight's Batch", allowing the user to review, swap, or cancel specific outreach tasks.
- **6:30 PM (Execution Phase)**: The automation service starts and processes only items in the `queued` status.
...
- **Effect**: Clicking "Stop Firm" marks the firm as `blocked` and transitions all related `pending`/`queued` tasks to `stopped`.
...
- Update `LinkedInService.ts` to support the new `linkedin_tasks` status flow.
- Update `QueueService.ts` to include the 6:00 PM `Batching` phase and 6:30 PM execution.
- Implement the `FOR UPDATE SKIP LOCKED` query for concurrency safety.
