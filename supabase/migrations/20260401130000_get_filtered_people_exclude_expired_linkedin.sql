-- Update get_filtered_people_paginated: p_has_linkedin excludes expired LinkedIn profiles.
--
-- This file is a copy of 20260327130000_fix_people_stable_sorting.sql with ONE intentional change:
-- the p_has_linkedin true/false branches join linkedin_profiles and require
-- COALESCE(lp.is_expired, false) = false so "Has LinkedIn" matches only non-expired links.
--
-- Depends on: 20260401120000_add_linkedin_profile_expiration.sql (linkedin_profiles.is_expired).
...
BEGIN
  ...
  -- Full Text Search with Prefix Matching logic
  IF p_search_query IS NOT NULL AND trim(p_search_query) != '' THEN
    DECLARE
      v_clean_query text;
      v_ts_query text;
    BEGIN
      -- 1. Remove special characters that break to_tsquery
      v_clean_query := regexp_replace(trim(p_search_query), '[!&|()<>:]', ' ', 'g');
      
      -- 2. Split by spaces, add :* to each term, and join with &
      -- Example: 'John Sm' -> 'John:* & Sm:*'
      SELECT string_agg(lexeme || ':*', ' & ')
      INTO v_ts_query
      FROM unnest(regexp_split_to_array(trim(v_clean_query), '\s+')) AS lexeme
      WHERE lexeme != '';

      IF v_ts_query IS NOT NULL THEN
        where_clauses := array_append(where_clauses, format('p.search_vector @@ to_tsquery(''english'', %L)', v_ts_query));
      END IF;
    END;
  END IF;
  ...
  IF p_has_linkedin = true THEN
    where_clauses := array_append(
      ...
      'EXISTS (
         SELECT 1
         FROM person_linkedin pl
         JOIN linkedin_profiles lp ON lp.id = pl.linkedin_id
         WHERE pl.person_id = p.id AND COALESCE(lp.is_expired, false) = false
       )'
    );
