-- Redefine get_filtered_people_paginated with prefix matching support
...
BEGIN
  ...
  -- Get page of IDs
  EXECUTE 'SELECT array_agg(id) FROM (SELECT p.id ' || base_query || ' GROUP BY p.id, p.created_at ORDER BY p.created_at DESC LIMIT ' || p_limit || ' OFFSET ' || p_offset || ') sub' INTO v_ids;
