-- Migration: Fix create_contact_full RPC type error (v3)
--
-- This migration fixes the "operator does not exist: record ->> unknown" error
-- by declaring contact_method as jsonb instead of record.
...
DECLARE
    ...
    new_org_id uuid;
    contact_method jsonb; -- Changed from record to jsonb
BEGIN
    ...
    IF payload ? 'emails' THEN
        FOR contact_method IN SELECT jsonb_array_elements(payload->'emails') LOOP
    ...
    IF payload ? 'phones' THEN
        FOR contact_method IN SELECT jsonb_array_elements(payload->'phones') LOOP
    ...
    IF payload ? 'linkedin' THEN
        FOR contact_method IN SELECT jsonb_array_elements(payload->'linkedin') LOOP
